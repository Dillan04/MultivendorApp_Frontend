...
const Wishlist = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const profileResponse = await fetch(`https://multivendorapp-user-microservice.onrender.com/profile`, {
          method: 'GET',
          headers: {
        ...
        const wishlistDetails = await Promise.all(
          wishlistIds.map(async (id) => {
            ...
            const itemResponse = await fetch(`https://multivendorapp-user-microservice.onrender.com/${id}`, {
              method: 'GET',
              headers: {
  ...
  return (
    <div className="min-h-screen bg-white py-12 px-6">
      <h2 className="text-3xl font-semibold text-center text-black mb-6">Your Wishlist</h2>

      {error && <p className="text-red-500 text-center mb-6">{error}</p>}

      {wishlist.length === 0 ? (
        <p className="text-center text-xl text-black/75">Your wishlist is empty, add items.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {wishlist.map((item) => (
            <div
              key={item._id}
              className="bg-white shadow-lg rounded-2xl p-6 transform transition-all duration-300 hover:scale-105 hover:shadow-xl border-2 border-orange-500"
            >
              <img
              ...
              <p className="text-black/75 mb-4">{item.description}</p>
