...
const handleImageUpload = async (event) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError('You are not the seller of this item therefore you do not have the permission to modify this product.');
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg">
      <h2 className="text-3xl font-bold text-orange-500 mb-6">{id ? 'Edit Current Product' : 'Create a new Product'}</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <input
            ...
            required
            className="w-full p-3 border border-orange-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2"
          />
        </div>
        ...
        <div>
          <label htmlFor="available" className="block text-sm font-medium text-orange-700">
            Available
          </label>
          <input
            id="available"
        ...
        <button
          type="submit"
          className="w-full py-3 bg-orange-500 text-white rounded-md hover:bg-orange-600 transition-all"
        >
          {id ? 'Update Current Product' : 'Create New Product'}
        </button>
      </form>
    </div>
  );
